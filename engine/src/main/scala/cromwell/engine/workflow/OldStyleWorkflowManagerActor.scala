...
object OldStyleWorkflowManagerActor {
  ...
  final case class RestartWorkflows(workflows: List[OldStyleWorkflowDescriptor]) extends WorkflowManagerActorMessage
  ...
  def props(): Props = Props(new OldStyleWorkflowManagerActor())
 ...
 */
@deprecated(message = "This class will not be part of the PBE universe", since = "May 2nd 2016")
class OldStyleWorkflowManagerActor(config: Config)
  extends LoggingFSM[WorkflowManagerState, WorkflowManagerData] with CromwellActor {

  def this() = this(OldStyleWorkflowManagerActor.defaultConfig)
  ...
  private def replyToAbort(id: WorkflowId): Unit = {
    ...
    globalDataAccess.getWorkflowState(id) onComplete {
      ...
      case Success(Some(s)) =>
        workflowActor foreach { _ ! OldStyleWorkflowActor.AbortWorkflow }
  ...
  private def replyToQuery(uri: Uri, rawParameters: Seq[(String, String)]): Unit = {
    ...
    query(rawParameters) onComplete {
      case Success(r) => sndr ! WorkflowManagerQuerySuccess(uri, r)
  ...
  private def assertCallFqnWellFormed(descriptor: OldStyleWorkflowDescriptor, callFqn: FullyQualifiedName): Future[Unit] = {
  ...
  private def submitWorkflow(source: WorkflowSourceFiles, replyTo: Option[ActorRef], id: Option[WorkflowId]): Future[WorkflowIdToActorRef] = {
    ...
    val message  = OldStyleMaterializeWorkflowDescriptorActor.MaterializeWorkflow(workflowId, source, config)
    val materializeWorkflowDescriptorActor = context.actorOf(OldStyleMaterializeWorkflowDescriptorActor.props(), name = s"MaterializeWorkflowDescriptorActor-$workflowId")
    materializeWorkflowDescriptorActor.askNoTimeout(message) map {
      case MaterializeWorkflowDescriptorSuccess(descriptor) =>
        val wfActor = context.actorOf(OldStyleWorkflowActor.props(descriptor), name = s"WorkflowActor-$workflowId")
  ...
  private def restartWorkflow(restartableWorkflow: OldStyleWorkflowDescriptor): Future[WorkflowIdToActorRef] = {
