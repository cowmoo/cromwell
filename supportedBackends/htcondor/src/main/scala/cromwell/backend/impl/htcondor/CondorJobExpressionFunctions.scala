...
object CondorJobExpressionFunctions {
  private val LocalFSScheme = "file"
  def isLocalPath(path: Path) = path.toUri.getScheme == CondorJobExpressionFunctions.LocalFSScheme
  def apply(workflowDescriptor: BackendWorkflowDescriptor, jobKey: BackendJobDescriptorKey, configurationDescriptor: BackendConfigurationDescriptor) = {
    ...
    new CondorJobExpressionFunctions(CondorJobExecutionActor.fileSystems, callContext)
  }
...
class CondorJobExpressionFunctions(override val fileSystems: List[FileSystem],
                                   context: CallContext
                                 ) extends WdlStandardLibraryFunctions with PureFunctions with ReadLikeFunctions with WriteFunctions {
  import CondorJobExpressionFunctions._
